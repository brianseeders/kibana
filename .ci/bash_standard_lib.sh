...
# If a matching local refrence repo is found, then that will be used as the `--reference` for the Git clone,
...
clone_git_repo() {
  ...
  if [[ $repo == git* ]]; then
    ...
    if [[ -d $mirror_dir ]]; then
      ...
      printf 'Attempting to clone %s using the short-name, however no matching refrence repo found. Exiting...\n' "$repo" >&2
      exit 1
    fi
  fi
